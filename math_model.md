...
$$
x(t) = \begin{bmatrix} x(t) \\ y(t) \\ z(t) \\ v_x(t) \\ v_y(t) \\ v_z(t) \\ m(t) \end{bmatrix} \in \mathbb{R}^7
$$
...
$$
T(t) = \begin{bmatrix} T_x(t) \\ T_y(t) \\ T_z(t) \end{bmatrix} \in \mathbb{R}^3, \qquad T(t) = \text{thrust vector [N]}
$$
...
$$
m_{\text{dry}} \le m(t) \le m_0, \qquad T_{\min} \le \|T(t)\|_2 \le T_{\max}
$$
...
$$
\hat{v} = \begin{cases} v / \|v\|, & \|v\| > 0 \\ 0, & \|v\| = 0 \end{cases}
$$

Aerodynamic drag is modeled as

$$
D(t) = -\frac{1}{2}\,\rho(z)\,C_d\,A\,\|v\|^2\,\hat{v} \in \mathbb{R}^3
$$
...
$$
T(t) = \begin{bmatrix} 0 \\ 0 \\ T_z \end{bmatrix}, \quad 0 \le t \le t_f
$$
...
k_4 &= f\left(t_k + h,\, x_k + h k_3\right) \\
x_{k+1} &= x_k + \frac{h}{6}\left(k_1 + 2k_2 + 2k_3 + k_4\right)
\end{aligned}
$$

In this project $h = \Delta t$ is typically $0.02\ \text{s}$ to $0.05\ \text{s}$.
...
$$
T_i = \begin{bmatrix} T_{x,i} \\ T_{y,i} \\ T_{z,i} \end{bmatrix}
$$
...
$$
J_{\text{classical}} = 1000\,\|r_f\|_2^2 + 1000\,\|v_f\|_2^2 + (m_0 - m_f)
$$
...
$$
\theta = \begin{bmatrix} T_0^\top & T_1^\top & \dots & T_{N-1}^\top \end{bmatrix}^\top \in \mathbb{R}^{3N}
$$
...
$$
e^{(i)}_{\text{pos}} = \|r_f^{(i)}\|_2, \qquad e^{(i)}_{\text{vel}} = \|v_f^{(i)}\|_2
$$

Define success indicator

$$
\mathbb{I}^{(i)}_{\text{succ}} = \begin{cases} 1, & e^{(i)}_{\text{pos}} < 5\ \text{m and } e^{(i)}_{\text{vel}} < 2\ \text{m/s} \\ 0, & \text{otherwise} \end{cases}
$$

The estimated success rate is

$$
\hat{p}_{\text{succ}} = \frac{1}{N_{\text{trials}}} \sum_{i=1}^{N_{\text{trials}}} \mathbb{I}^{(i)}_{\text{succ}}
$$
...
- $r_k \in \mathbb{R}^3$ — position at step $k$
- $v_k \in \mathbb{R}^3$ — velocity at step $k$
- $z_k \in \mathbb{R}$ — log mass at step $k$
- $u_k \in \mathbb{R}^3$ — acceleration control at step $k$
- $\Gamma_k \in \mathbb{R}$ — thrust-per-mass magnitude at step $k$
...
$$
\|u_k\|_2 \le \Gamma_k, \quad k = 0,\dots,N-1
$$
...
$$
\Gamma_{\min} = \frac{T_{\min}}{m_0}, \qquad \Gamma_{\max} = \frac{T_{\max}}{m_{\text{dry}}}
$$
...
$$
e_{\text{pos}} = \|r(t_f)\|_2, \qquad e_{\text{vel}} = \|v(t_f)\|_2
$$
